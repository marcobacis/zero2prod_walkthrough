...
pub async fn try_processing(
    ...
    if inserted_rows > 0 {
        ...
        let saved_response = get_saved_response(pool, key, user_id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("expected a saved http response but didn't find it"))?;
...
pub async fn save_response(
    ...
    let response_status = head.status().as_u16() as i16;
